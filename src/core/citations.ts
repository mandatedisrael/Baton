...
import type { Citation, Handoff } from "../schema/handoff.ts";

export interface CitedClaim {
  kind: "decision" | "graveyard";
  id: string;
  summary: string;
  citation: Citation;
}
...
export function findCitedClaim(handoff: Handoff, claimId: string): CitedClaim {
  const decision = handoff.decisions.find((item) => item.id === claimId);
  if (decision) {
    if (!decision.citation) throw new BatonError("NOT_FOUND", `decision ${claimId} has no source citation`);
...
export function extractCitationSpan(source: Uint8Array | string, citation: Citation): string[] {
  ...
  if (citation.fromLine < 1 || citation.toLine > lines.length || citation.toLine < citation.fromLine) {
    throw new BatonError(
      "INVALID_HANDOFF",
      `citation lines ${citation.fromLine}-${citation.toLine} fall outside attachment (${lines.length} lines)`,
    );
  }
  return lines.slice(citation.fromLine - 1, citation.toLine);
}

export function renderCitationEvidence(claim: CitedClaim, lines: string[]): string {
