...
## Live Codex-to-Claude transcript handoff evidence

On 2026-06-20, Baton captured the active Codex Desktop rollout for this repository by matching its recorded project `cwd`. The review gate disclosed a 3,193,100-byte, 840-line transcript, two scrubbed assignment-like secret findings, the structured decisions and graveyard, and the exact remote delivery behavior before aborting without sealing. A subsequent explicitly confirmed pass created transcript-mode baton `741f2b6f507deeb158b6e92943da7f3f8530752eb9294c3ee7780ef1316bb6d0` with source tool `codex`.

Seal encrypted the canonical handoff and transcript as two independent payloads. An interruption after Walrus certification but before the local completion checkpoint exposed an ambiguous-success retry bug; Baton now queries the registered blob object, verifies its blob identity, and treats a non-null certified epoch as completed rather than submitting certification twice. The resumed publication anchored successfully in Sui transaction `8VXndc1LL5oyoxwu8uSeJzRBNKKMsjmxkX4qtAheFDM1`.

A fresh Ed25519 recipient identity `0x647fce3039adf2733d5d1577ad0a74577ef86d1f6366475a16966e0a57dd53df` received access in transaction `5xd5tfdpxhanp2XMno8DTSnEk16VZ1qxA1tgz5MY7nzD`, accepted the invitation in an empty project, fetched both ciphertexts from Walrus, decrypted them through Seal, verified the Sui manifest and every plaintext hash, and rendered a Claude Code resume prompt with `Lineage: 741f2b6f507d (codex)` and `Source: codex · capture transcript`. Owner-side `baton audit` independently authenticated the two payloads and 3,202,863 plaintext bytes without writing recovered content.

After revocation, the recipient's cached handoff and attachment were removed and a clean fetch failed with `User does not have access to one or more of the requested keys`; neither plaintext file was recreated. This proves real Codex transcript capture and cryptographically authorized Codex-to-Claude transport between isolated identities on Testnet. The two identities ran on one physical machine, and fidelity remained ungraded because no Anthropic API key was configured; separate-machine beta behavior and citation-quality grading therefore remain unproven.
