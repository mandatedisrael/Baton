 ...
 */
import { DEFAULT_MODEL, LLMError } from "./client.js";
...
export class AnthropicClient {
    ...
    async send(payload) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            if (attempt > 0)
                await this.sleep(this.backoff(attempt, lastErr));
            let res;
            try {
    ...
    backoff(attempt, lastErr) {
        const hint = lastErr && typeof lastErr === "object" && "retryAfter" in lastErr
            ? Number(lastErr.retryAfter)
            : NaN;
