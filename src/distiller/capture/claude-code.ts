...
function flatten(message: Record<string, unknown>): FlatContent {
  const text: string[] = [];
  ...
  if (typeof content === "string") {
    ...
    for (const raw of content) {
      ...
      switch (it.type) {
        case "text":
          if (typeof it.text === "string") text.push(it.text);
          break;
        case "thinking":
          if (typeof it.thinking === "string") thinking.push(it.thinking);
          break;
        case "tool_use":
          toolUses.push({
...
export function parseClaudeCodeTranscript(raw: string): CapturedSession {
...
export function captureClaudeCodeFile(path: string): CapturedSession {
  let raw: string;
  try {
