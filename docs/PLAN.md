...
Walrus Sites lineage viewer (DAG, handoff browser, fidelity badges; diff view). Failure-mode hardening — sponsored registration now persists its deterministic transaction digest and submitted state before execution, reconciles ambiguous outcomes from Sui on retry/startup/operator command, retains submitted records across expiry, and blocks unsafe revoke/prune actions. Remaining failure work includes kill wifi mid-pass, corrupt a blob, and revoke mid-session; every failure must be graceful and *narratable*. Self-report mode against ChatGPT web + import path for export zips. Mainnet release. Beta with external devs; collect installs, batons passed, cross-tool resumes, mean fidelity.
