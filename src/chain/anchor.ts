...
function uploadedBlob(job: UploadJob, id: string) {
...
function byteVector(value: unknown): Uint8Array | null {
  if (!Array.isArray(value) || value.some((item) => !Number.isInteger(item) || item < 0 || item > 255)) return null;
  return Uint8Array.from(value as number[]);
}

/** Verify a previously created dynamic field before treating an interrupted anchor as complete. */
export function extractExistingAnchor(
  ...
  const name = (fields.name as { fields?: Record<string, unknown> } | undefined)?.fields;
  const value = (fields.value as { fields?: Record<string, unknown> } | undefined)?.fields;
...
async function existingAnchorDigest(
...
export async function anchorQueuedJob(input: {
  ...
  if (job.status === "complete") return { job, sidecar: input.store.loadRemoteSidecar(input.handoffId) };
