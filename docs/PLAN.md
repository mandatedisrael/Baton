...
Raw-keypair delegated access is complete on Testnet: non-transferable address-bound `AccessCap`s, live recipient verification, generation-safe re-granting, invitation acceptance, delegated recovery, and revocation enforced by Seal key servers. Sponsored registration is also proven on Testnet with zero-balance users: user and sponsor sign identical bytes, the sponsor reconstructs only the allowed `create_project` call, concrete gas coins are reserved, and invitations are hashed and one-use. Operator controls now support recipient/project pre-binding, live inspection, pre-use revocation, terminal-record pruning, durable used-result audits, and cross-process serialization of daemon and CLI state changes. Remaining: zkLogin onboarding through a Sui-supported OAuth provider, a public sponsor deployment with production TLS/monitoring/abuse operations, and broader external multi-user Journey C testing.
