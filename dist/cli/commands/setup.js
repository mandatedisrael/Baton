...
export function currentMcpLaunch() {
    ...
    if (!existsSync(mcp))
        throw new BatonError("NOT_FOUND", `Baton MCP entrypoint is missing: ${mcp}`);
    return {
...
function upsertCodex(root, launch) {
    ...
    if (begin === -1 && /^\s*\[mcp_servers\.baton\]\s*$/m.test(existing)) {
        throw new BatonError("ALREADY_INITIALIZED", `${path} already defines mcp_servers.baton outside Baton's managed block; remove or rename it first`);
    }
...
function upsertJsonMcp(path, root, launch) {
    mkdirSync(dirname(path), { recursive: true });
    let config = {};
    if (existsSync(path)) {
        try {
...
export function setupAgents(root, agents, launch = currentMcpLaunch()) {
    ...
    for (const agent of agents) {
        if (agent === "codex")
            paths.push(upsertCodex(root, launch));
