...
const token = (type: string): string => `[REDACTED:${type}]`;
...
interface Pattern {
  type: string;
  re: RegExp;
  /** Replacement; defaults to the redaction token. `$1` etc. are honored. */
...
const ASSIGNMENT =
  /(\b(?:api[_-]?key|apikey|secret|secret[_-]?key|access[_-]?key|client[_-]?secret|password|passwd|pwd|auth[_-]?token|token|bearer)\b\s*[:=]\s*)(["']?)([^\s"']{12,})(["']?)/gi;
  ...
  const cleaned = lines.map((line) => {
    ...
    for (const { type, re, replace } of PATTERNS) {
      out = out.replace(re, (...args) => {
        bump(type);
        return replace ? replace(type).replace("$1", args[1] as string) : token(type);
      });
    }

    out = out.replace(ASSIGNMENT, (whole, prefix: string, openQ: string, value: string, closeQ: string) => {
      ...
      if (shannonEntropy(value) < 2.5) return whole;
      bump("assignment");
...
function mergeFindings(into: Map<string, number>, from: ScrubFinding[]): void {
  for (const f of from) into.set(f.type, (into.get(f.type) ?? 0) + f.count);
}

export interface ScrubDeepResult {
  value: unknown;
  findings: ScrubFinding[];
}
 ...
 */
export function scrubDeep(value: unknown): ScrubDeepResult {
  const counts = new Map<string, number>();
