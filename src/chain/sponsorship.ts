...
export async function sponsoredTransactionDigest(bytes: Uint8Array): Promise<string> {
  try {
...
function registrationResultFromResponse(
  response: Awaited<ReturnType<SuiJsonRpcClient["getTransactionBlock"]>>,
  typePackageId: string,
...
export async function reconcileSponsoredRegistration(input: {
  ...
  let response: Awaited<ReturnType<SuiJsonRpcClient["getTransactionBlock"]>>;
  try {
