...
import { HANDOFF_STATUSES, } from "../schema/handoff.js";
import { DEFAULT_MODEL } from "../llm/client.js";
import { extractJsonObject } from "./json.js";
const MAX_FIELD = 2000;
function truncate(s, max = MAX_FIELD) {
...
function renderDelta(delta) {
    ...
    for (const m of delta) {
        if (m.isMeta)
            continue; // tool-injected reminders aren't session content
        const head = `[line ${m.line}] ${m.role}`;
        if (m.thinking)
            out.push(`${head} (thinking): ${truncate(m.thinking)}`);
        if (m.text)
            out.push(`${head}: ${truncate(m.text)}`);
        for (const t of m.toolUses)
            out.push(`${head} → tool ${t.name}(${truncate(JSON.stringify(t.input ?? {}), 400)})`);
...
// ---------------------------------------------------------------------------
const str = (o, k) => typeof o[k] === "string" && o[k].length > 0 ? o[k] : undefined;
function citationOf(raw, transcriptLineCount, attachmentId) {
    if (!attachmentId || !raw || typeof raw !== "object")
        return undefined;
    const c = raw;
    ...
    if (!Number.isInteger(from) || !Number.isInteger(to))
        return undefined;
    if (from < 1 || to < from || to > transcriptLineCount)
        return undefined; // out of range → drop
    return { attachmentId, fromLine: from, toLine: to };
}
function buildOp(raw, ctx) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
        return null;
    const o = raw;
    const cite = (k = "citation") => citationOf(o[k], ctx.transcriptLineCount, ctx.attachmentId);
    switch (o.kind) {
        case "set_mission": {
        ...
        case "set_status": {
            ...
            return typeof status === "string" && HANDOFF_STATUSES.includes(status)
        ...
        case "add_decision": {
            ...
            if (!choice)
                return null;
            const decision = { id: `d-${randomUUID()}`, choice, rationale: str(o, "rationale") ?? "" };
            const citation = cite();
            if (citation)
        ...
        case "move_to_graveyard": {
            ...
            if (!approach)
                return null;
            const entry = { id: `g-${randomUUID()}`, approach, reason: str(o, "reason") ?? "" };
            const citation = cite();
            if (citation)
        ...
        case "set_next_actions": {
            if (!Array.isArray(o.actions))
                return null;
            const actions = o.actions.filter((a) => typeof a === "string" && a.length > 0);
...
export function parseExtractionResponse(text, ctx) {
...
export async function extractCheckpoint(client, input, opts = {}) {
