...
import { z } from "zod";
import { listHandoffs, searchHandoffs } from "../core/query.ts";
import { buildResumePrompt } from "../core/resume.ts";
import { TOOL_IDS } from "../schema/handoff.ts";
import { ProjectStore } from "../store/project.ts";
import { verificationEvidenceFromStore } from "../cli/commands/verify.ts";
import { ensureHandoffAvailable, recoverHandoffFromRemote } from "../cli/remote.ts";
import { resolveHandoffId } from "../cli/resolve.ts";
...
function textAndStructured(text: string, structuredContent: Record<string, unknown>) {
  return { content: [{ type: "text" as const, text }], structuredContent };
}

async function resolveAvailable(store: ProjectStore, idPrefix?: string) {
...
export function registerReadTools(server: McpServer, projectDir: string): void {
  ...
  server.registerTool("baton_show", {
    ...
    const structuredHandoff: Record<string, unknown> = { ...handoff };
