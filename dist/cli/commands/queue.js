import { renderQueueStatus } from "../../render/queue.js";
...
export async function runQueueEncrypt(cwd) {
    ...
    const pending = store.listUploadJobs().filter((job) => job.blobs.some((blob) => blob.status === "pending"));
    if (pending.length === 0) {
    ...
    for (const queued of pending) {
        const job = await encryptQueuedJob(store, queued.handoffId, encryptor, {
...
export async function runQueueUpload(cwd, identityPath) {
    const store = ProjectStore.open(cwd);
    const remote = store.config().remote;
    if (!remote) {
        throw new BatonError("INVALID_STATE", "project is local-only — run `baton login` then `baton register`");
    ...
    if (failed > 0)
        throw new BatonError("IO_ERROR", `${failed} publication job(s) could not be uploaded`);
}
export async function runQueueAnchor(cwd, identityPath) {
    const store = ProjectStore.open(cwd);
    const remote = store.config().remote;
    if (!remote) {
        throw new BatonError("INVALID_STATE", "project is local-only — run `baton login` then `baton register`");
    ...
    if (failed > 0)
        throw new BatonError("IO_ERROR", `${failed} publication job(s) could not be anchored`);
}
