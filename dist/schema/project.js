...
export const SUI_NETWORKS = ["testnet", "mainnet"];
export const REMOTE_AUTHORITY_KINDS = ["owner", "delegate"];
function suiId(v, path) {
    const value = str(v, path);
    if (!/^0x[a-fA-F0-9]{1,64}$/.test(value))
        throw new ValidationError(path, "expected a Sui object id");
    return value.toLowerCase();
}
function httpUrl(v, path) {
...
function parseKeyServer(v, path) {
    ...
    if (aggregatorUrl !== undefined)
        result.aggregatorUrl = httpUrl(aggregatorUrl, `${path}.aggregatorUrl`);
    return result;
}
function parseRemote(v, path) {
    const r = obj(v, path, [
        "network",
        "rpcUrl",
        "packageId",
    ...
    const authority = r.authority === undefined
        ? { kind: "owner", capId: suiId(r.ownerCapId, `${path}.ownerCapId`) }
        : (() => {
    ...
    if (serverConfigs.length === 0)
        throw new ValidationError(`${path}.seal.serverConfigs`, "expected at least one server");
    ...
    const aggregatorUrl = httpUrl(walrusRaw.aggregatorUrl ?? "https://aggregator.walrus-testnet.walrus.space", `${path}.walrus.aggregatorUrl`);
...
export function parseProjectConfig(v) {
