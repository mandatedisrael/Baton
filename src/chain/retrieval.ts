...
export interface WalrusRetriever {
  fetch(blobId: string): Promise<Uint8Array>;
}

export function validateWalrusBlobId(blobId: string): string {
    ...
    async fetch(blobId) {
      ...
      if (!response.ok) {
        throw new BatonError(
          response.status === 404 ? "NOT_FOUND" : "IO_ERROR",
          `Walrus aggregator returned HTTP ${response.status} for ${blobId}`,
        );
      }
...
export async function fetchVerifiedCiphertext(input: {
  ...
  if (input.encryptedHash !== undefined) {
    ...
    if (actual !== input.encryptedHash) {
      throw new BatonError(
        "HASH_MISMATCH",
        `Walrus ciphertext ${input.blobId} hashes to ${actual}, expected ${input.encryptedHash}`,
      );
    }
  }
  return bytes;
