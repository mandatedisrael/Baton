...
import { isoDatetime, literal, obj, str, nullable } from "../schema/validate.js";
export function defaultIdentityPath(home = homedir()) {
...
function parseIdentity(value) {
    ...
    const scheme = literal(r.scheme, "identity.scheme", "ED25519");
    ...
    if (scheme === "ZKLOGIN") {
        return {
            ...
            scheme: "ZKLOGIN",
            address,
            provider: literal(r.provider, "identity.provider", "google"),
...
export function loadIdentity(path = defaultIdentityPath()) {
    if (!existsSync(path))
        throw new BatonError("NOT_FOUND", `no Baton identity at ${path} — run \`baton login\``);
    try {
...
export function loadOrCreateIdentity(path = defaultIdentityPath(), now = new Date()) {
    if (existsSync(path))
...
export function saveZkLoginIdentity(session, path = defaultIdentityPath(), now = new Date()) {
    const dir = dirname(path);
    mkdirSync(dir, { recursive: true, mode: 0o700 });
    chmodSync(dir, 0o700);
    const record = {
...
export function isZkLoginIdentity(loaded) {
...
export function requireEd25519Identity(loaded) {
    if (loaded.scheme !== "ED25519") {
        throw new BatonError("INVALID_STATE", "This command requires an Ed25519 identity for now (Seal or certain Walrus ops). Use `baton login` (without --zk) for a raw keypair, or a separate ED identity for these ops.");
    }
    return loaded;
}
/** Get the Sui address for any identity type (ED or ZK). */
export function getIdentityAddress(loaded) {
 ...
 */
export function getEd25519Keypair(loaded) {
    if (loaded.scheme === "ED25519") {
