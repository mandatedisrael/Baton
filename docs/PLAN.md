...
**Status:** Phases 1–4 are complete. Two raw-keypair Phase 5 slices are live on Testnet: delegated sharing/revocation and operationally hardened sponsored registration for zero-balance users. A supported-provider zkLogin flow, a public sponsor deployment, and broader external multi-user testing remain.
...
**Phase 2 — Distiller** *(done for Claude Code and Codex transcript capture; Cursor remains integration work)*
Claude Code hooks → transcripts. Codex project-scoped rollout discovery → normalized transcript capture at pass. The micro-checkpoint loop: event hooks or MCP self-report → WorkingState patch ops. Pass-as-commit finalization. Secrets scrubber over the distillate and source. Durable content-addressed attachments. Fidelity grader + rubric v1 when an Anthropic API key is configured. Review-gate UX discloses transcript size, scrub findings, and delivery before sealing. Dialect renderer + resume prompts. Deterministic fallback and `baton verify` cross-examination. **Dogfood from the start: build BATON using BATON.** Tracked metrics: graveyard recall (does the handoff contain what actually failed?) and fidelity-score distribution.
...
Raw-keypair delegated access is complete on Testnet: non-transferable address-bound `AccessCap`s, live recipient verification, generation-safe re-granting, invitation acceptance, delegated recovery, and revocation enforced by Seal key servers. Sponsored registration is also proven on Testnet with zero-balance users: user and sponsor sign identical bytes, the sponsor reconstructs only the allowed `create_project` call, concrete gas coins are reserved, and invitations are hashed and one-use. Operator controls now support recipient/project pre-binding, live inspection, pre-use revocation, terminal-record pruning, durable used-result audits, cross-process serialization, chain-aware readiness, counts-only metrics, trusted-proxy throttling, and explicit daily/concurrent liability caps. Remaining: zkLogin onboarding through a Sui-supported OAuth provider, an actual public sponsor deployment with managed TLS/monitoring/abuse operations, and broader external multi-user Journey C testing.

**Phase 6 — Viewer & hardening**
Walrus Sites lineage viewer (DAG, handoff browser, fidelity badges; diff view). Failure-mode hardening — sponsored registration now persists its deterministic transaction digest and submitted state before execution, reconciles ambiguous outcomes from Sui on retry/startup/operator command, retains submitted records across expiry, and blocks unsafe revoke/prune actions. `baton audit` authenticates an anchored baton and every attachment across Sui, Walrus, and Seal without changing local state; live corruption and revoked-access probes fail closed, and recovery persists nothing if any payload fails. Remaining failure work includes broader network-loss timing across publication stages and arbitrary instruction-boundary termination; every failure must be graceful and *narratable*. Self-report mode against ChatGPT web + import path for export zips. Mainnet release. Beta with external devs; collect installs, batons passed, cross-tool resumes, mean fidelity.
