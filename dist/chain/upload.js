...
const STEP_ORDER = ["encoded", "registered", "uploaded"];
export function beginUploadAttempt(job, now = new Date()) {
...
export function markWalrusCheckpoint(job, queueBlobId, checkpoint, now = new Date()) {
    const target = job.blobs.find((blob) => blob.id === queueBlobId);
    if (!target)
        throw new BatonError("NOT_FOUND", `upload job has no blob ${queueBlobId}`);
...
export function markBlobUploaded(job, queueBlobId, blobId, now = new Date()) {
    const target = job.blobs.find((blob) => blob.id === queueBlobId);
    if (!target)
        throw new BatonError("NOT_FOUND", `upload job has no blob ${queueBlobId}`);
    ...
    const blobs = job.blobs.map((blob) => blob.id === queueBlobId ? { ...blob, status: "uploaded", blobId } : blob);
    return parseUploadJob({
        ...job,
...
export async function uploadQueuedJob(store, handoffId, uploader, now = new Date()) {
