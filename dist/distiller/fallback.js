...
function unquoteGitPath(p) {
    if (p.startsWith('"') && p.endsWith('"')) {
        try {
            return JSON.parse(p); // git uses C-style escapes, JSON-compatible enough
        }
        catch {
            return p.slice(1, -1);
        }
...
export function parseGitStatusPorcelain(out) {
    const paths = [];
...
export function extractCheckboxTodos(markdown, max = 10) {
    const todos = [];
...
export function fallbackPatchOps(signal) {
    const ops = [];
    if (signal.touched.length > 0)
        ops.push({ kind: "touch_files", files: signal.touched });
    if (signal.nextActions.length > 0)
        ops.push({ kind: "set_next_actions", actions: signal.nextActions });
    for (const note of signal.envNotes)
        ops.push({ kind: "add_env_note", note });
    return ops;
}
...
export function gatherFallbackSignal(cwd) {
    ...
    if (status !== null) {
        for (const rel of parseGitStatusPorcelain(status)) {
            ...
            if (rel === ".baton" || rel === ".baton/" || rel.startsWith(".baton/"))
                continue;
